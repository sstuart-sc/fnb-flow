...
11. **Vue.js migration — new, not yet started, comes after item 9.** The POC's original vanilla ES-modules + hand-rolled hash router + manual re-render approach (phase 6) was the right call to prove the concept fast, but now that the site has real traction, it's worth converting to a proper framework. Sequenced *after* item 9 (not before) so the large data audit/expansion happens first against the existing, already-verified rendering code — the data model and content are what's still actively changing; the rendering layer should be migrated once that churn has settled, not concurrently with it.
    - **Intent:** move the viewer to Vue 3 (Composition API, `<script setup>`) using current idiomatic practices — Vite as the build tool, Vue Router replacing the hand-rolled `router.js` hash parser, reactive state (via `ref`/`reactive` or Pinia if state sharing across components warrants it) replacing manual re-render calls, and single-file components replacing the current `render-*.js` function-per-view files. Keep the app a static, servable build (Vite's output), preserving the "point any static host at the output folder" deploy story from item 7.
    - **Concise plan:**
      1. Scaffold a Vite + Vue 3 project alongside the existing `app/`, wire up `npm run dev`/`npm run build`.
      2. Port the data layer first (`data.js`'s fetch/cache/path-id resolution) as a composable, unchanged in behavior — no data-shape changes needed, this is a rendering-layer migration only.
      3. Convert one view at a time, easiest/highest-value first: primary flow view, then step detail, then nested flow, then the detail modal, then search, then sources page — verifying each in the browser before moving to the next so the migration stays bisectable.
      4. Re-implement routing last (once views exist to route between), swapping `router.js` for Vue Router with equivalent hash-based routes so existing bookmarked URLs keep working.
      5. Carry over CSS largely as-is (already theme-aware, not framework-specific) — scope it into components incrementally rather than rewriting it.
      6. Delete the old `app/js/*.js` vanilla files only once the Vue equivalent is verified and the old file's route/view is no longer reachable.
    - Not scoped further yet (build tool version pins, whether to adopt Pinia vs. plain composables, TypeScript or not) — those are implementation decisions to make when this item is actually picked up, not now.
