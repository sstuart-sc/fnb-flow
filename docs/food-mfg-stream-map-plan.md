...
> + data (`app/data/core`, `app/data/products`), `docs/` holds this plan.
...
3. **Sanity-check the nested-flow gating table (section 3)** against real SafetyChain product knowledge — flagged as unconfirmed from the start and still is, independent of the 7-step restructure (which only changed *where* the old 11 steps live, not whether each one still deserves full nested-flow depth). Particularly: is outbound distribution & logistics really trace-record-only, and does every one of the 11 now-nested children actually warrant full implementation depth today? (This question folds into item 8 below once that phase starts — an audit pass is exactly where this gets re-adjudicated for real.)
...
6. **Visual polish, if wanted** — current viewer intentionally kept to plain flexbox nodes + arrow glyphs and CSS-only interactions (no animation library wired in, despite Motion One being discussed as an option). Revisit only if the current look feels insufficient; a deliberate "keep it simple, discover as we go" call, not an oversight. Might be worth revisiting now that the deepest drill path is 6 breadcrumb levels.
...
8. **Source/citation tracking — new, not yet started.** The original 9 `docs/research` modules that grounded the first pass of content no longer exist as a fixed reference set (moved past deliberately, per user direction — this project isn't tied to re-deriving everything from those specific files going forward). Rather than re-anchoring on a new fixed doc set, the plan is a **traceability mechanism**: every future fact added to the data (and, as a separate, larger effort, every fact already in the data) should carry a pointer to what grounded it — a regulation's own text, a named industry standard (GS1, FSMA 204, GFSI), a vendor doc, or explicit subject-matter judgment where no external source exists — so the data stays auditable instead of just asserted. Concretely: a new `sources.json` core collection, plus an optional `sourceIds` field on the record types that carry designed/asserted content (`artifacts`, `materials`, `systems`, `steps` — not `regulations`/`agencies`/`roles`/`threads`/`flows`, which are either self-citing or structural). **Scope is explicitly split in two, same mechanism serving both:** (a) new content from here forward gets sourced as it's written — no exceptions; (b) auditing and back-filling sources on everything already in the data (all pre-existing artifacts/materials/systems/steps, including the 9 gap-resolution artifacts from this pass) is its own separate, large effort, tracked as item 9, not done retroactively as part of adding the mechanism itself.
9. **Large data audit & expansion phase — new, not yet started, comes after item 8's mechanism exists.** Once source tracking is in place, this becomes the next major body of work: revisit and expand the data far beyond what the original 9 modules covered, backfilling source citations on existing records as part of the same pass rather than a separate one. Expect this to be substantial — more artifacts, materials, systems, regulations, and possibly more nested-flow depth than exists today — and to surface real answers to the still-open gating question in item 3. Not scoped further yet; treat as the next major phase once the team is satisfied with current site mechanics (item 3's UI polish, thread filtering, etc. are independent and can proceed in parallel).
