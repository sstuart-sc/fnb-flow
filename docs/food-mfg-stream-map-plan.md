...
> where they appear. Item 8 (source/citation tracking mechanism) and item 9a
> (citation backfill + correctness audit across all existing content) are
> both done — see section 10. Item 10 (Implementation Detail card + modal
> upgrade) is done. Item 11 (Vue.js migration) is done — the viewer now lives
> at repo-root `src/`/`public/` (Vue 3 + Vite + Vue Router), not the old
> vanilla `app/` tree referenced further down this doc; treat `app/data/core`
> references below as `public/data/core` and `app/js/*` as historical (the
> logic they describe now lives in `src/composables/*` and `src/components/*`
> per [`CLAUDE.md`](../CLAUDE.md)). **Item 9 (large data audit & expansion) is
> now in progress** — a first full-dataset correctness/completeness audit
> pass is done (see section 10's newest entry): 15 real findings fixed
> (2 new agencies, 2 new regulations, 1 stale vendor rebrand, 3 systems with
> previously-empty vendor lists populated, 4 threads' `stepIds` reconciled to
> full bidirectional consistency, 2 genuine artifact-coverage gaps filled),
> zero referential-integrity errors remain. Expansion beyond what already
> exists (more artifacts/materials/systems/regulations, more nested-flow
> depth) is not yet started. See section 10 for the full progress log and
> section 11 for all next steps. `docs/content-standards.md` (item 8a, also
> covering section 3a's detail-field rules from item 10) — read that before
...
**Item 9 — first full-dataset audit pass (correctness/completeness) — done, browser-verified.** Kicked off item 9 (previously queued, not started) with an audit-first pass rather than jumping straight to new-content expansion, per the explicit instruction that the dataset must be verified healthy/trustworthy before adding features: audit everything for correctness/effectiveness/completeness, fix what's wrong, fill genuine gaps. Ran as a multi-agent workflow: 4 parallel research agents (agencies+regulations; artifacts; materials+systems; steps+flows+threads+roles), each doing real web lookups, followed by an independent adversarial-verification pass on every finding (a separate agent re-checking the file content and re-deriving the underlying fact from scratch, defaulting to reject unless it could independently confirm both). 16 findings surfaced, 15 confirmed, 1 correctly rejected (a `fields`-breakdown suggestion for `Scale` that the verifier found was already a deliberate, consistent exemption for all equipment-type materials).
- **Real gaps filled, not just corrections:** added **OSHA** and **CBP** to `agencies.json` (OSHA — workplace safety, general duty clause, PSM for ammonia refrigeration common in cold storage — was conspicuously absent from the whole agency roster despite being squarely in-scope for a plant-floor dataset; CBP's import-entry/detention-coordination role is distinct from FDA's importer-facing FSVP, already in the data). Added two regulations: the **2016 Nutrition Facts Label Final Rule** (81 FR 33742 — added-sugars declaration, updated Daily Values, revised serving sizes; a substantively different, later rule than the 1990 NLEA record already present, which only covered the original panel mandate) and the proposed (not yet final) **Front-of-Package Nutrition Information rule** (Federal Register 2025-00778), tracked the same way FSMA 204's compliance-date extension was tracked as in-flight before becoming binding.
- **Stale vendor info corrected:** `systems.json`'s QMS/Food Safety Software record still listed "Corvium" bare — corrected to "Neogen Analytics (formerly Corvium)" per Neogen's Feb 2023 acquisition, matching the file's existing rebrand-annotation convention (AVEVA MES formerly Wonderware, etc.). Populated three systems records whose `examplePlatforms` had been left empty despite real named vendors existing: **LIMS** (LabWare, STARLIMS, LabVantage, Thermo Fisher SampleManager), **Production Scheduling System** (PlanetTogether/CAI, Siemens Opcenter formerly Preactor), **Formulation/Recipe Management System** (Genesis R&D/ESHA/Trustwell, Vicinity Software).
- **Systemic referential-integrity gap found and fixed in `threads.json`:** all 4 threads' `stepIds` were a one-directional relationship — many leaf/nested-flow steps individually tagged a thread in their own `threadIds` but the thread's own `stepIds` never listed them back (Traceability & Lot Genealogy alone was missing ~24 steps this way; Sanitation & Cleaning missing 4; Allergen Control missing 12; Recall & Withdrawal missing 2). Fixed by scripting the union of each thread's declared `stepIds` with every step that independently tags it, confirmed against the audit's own itemized list before applying.
- **Two genuine artifact-coverage gaps filled:** `Primary & secondary packaging` (the packaging nested flow's sole leaf step with zero artifacts, while its 3 siblings — Fill & seal, Catch weight verification, Package integrity check — all carry at least one) got a new **Case/Pack Configuration Verification Record**; `FEFO management (finished goods)` (paired inconsistently against its comparable sibling `fg-inventory-traceability`, which does carry an artifact) got a new **FEFO Exception/Override Log**. Neither is grounded in an external regulation — both are honestly typed `subject-matter-judgment` in `sources.json` with an explicit note on why (standard secondary-packaging/WMS-override practice, not something any named rule mandates at this granularity), per `content-standards.md` §2's rule against force-fitting a citation that doesn't really apply.
- **Not changed, considered and left as-is:** the FDA front-of-package rule and CBP additions were flagged low-severity/optional in the audit (not-yet-final rule; import-boundary agency arguably outside the receiving-dock-to-shipping-dock slice) but added anyway since both are real, verifiable, and the dataset already sets precedent for tracking in-flight rules and border-adjacent context — judged worth the completeness over strict minimalism. `Scale`'s missing `fields` breakdown was investigated and confirmed a non-issue: all equipment-type materials are consistently `fields`-free by design.
- **Referential integrity re-validated end-to-end** with an extended version of the migration-era validation script (checked dict-key==id, every `*Ids` reference including `relatedIds` against artifacts/materials/systems/regulations, unique flow/step paths, unique thread keys, and transitive reachability of every step from `primary-flow`): zero dangling ids, all 68 steps still reachable, after all of the above changes.
- **Browser-verified** via a scripted Playwright pass (`chromium-cli` wasn't available in this environment, so a small script was run directly against `npx playwright`): primary flow view loads clean; the FEFO management step detail page renders the new FEFO Exception/Override Log artifact card with a working "Sourced (1)" link; the Primary & secondary packaging step (3 levels deep via the packaging grouping flow) renders the new Case/Pack Configuration Verification Record; the Sources page lists the new OSHA/CBP/Neogen/2016-NFL/FOP-proposed/LIMS-APS-formulation-vendor sources and its live filter narrows correctly on "Neogen." Zero console errors across all checks.

**Item 10 correction — dropped `example`, tightened `fields` for consistency — done, browser-verified.** User review of the just-shipped item 10 content caught two real quality problems in the entry below: (1) `fields` had been added inconsistently — 17 of 52 artifacts got a breakdown even when structurally comparable records (a Batch/Production Record, a CCP Monitoring Log, an Environmental Monitoring Record) didn't, because the original 7-batch backfill left the "does this need `fields`" call to per-batch agent judgment with only prose guidance; (2) `example` conflated narrative context and structured mock data into one free-text field, rendered as a plain paragraph, and different batches wrote it three different, inconsistent ways. Decision: `example` doesn't earn its complexity for an educational/conceptual reference site — a plausible, sourced *schema* (`fields`) is the actual bar, not a plausible-looking *instance* of one. Fixed rather than iterated on the format.
- **Schema/UI:** removed `example` entirely from `artifacts.json`/`materials.json`/`systems.json` (68 records had it) and from `render-detail-modal.js`'s rendering + its CSS (`.modal__example`); `render-step.js`'s `hasDetail` card-gate no longer checks for it.
- **Standard tightened:** `content-standards.md` §3a rewritten — `fields` is now a required test, not a judgment call: does the record represent a single describable instance with named parts? Default to yes for most `format` values in this dataset ("data record," "document, versioned," etc.) even when the existing `lifecycle` text doesn't spell out the structure; omit only for genuinely whole-document/narrative records with no enumerable parts.
- **Fields consistency re-pass:** ran a focused 3-batch workflow over the 36 artifacts still missing `fields` after the original backfill. 34 got a real, sourced breakdown (e.g. Receiving Log: shipment/receipt identifier, supplier/shipper, arrival date/time, quantity received, carrier/trailer reference, dock personnel); 2 were judged genuinely exempt and left without `fields` — **HACCP Plan** (a whole narrative document whose internal structure is what the CCP Definition artifact it contains already breaks out) and **Label Artwork File** (a finished design file with no schema distinct from the Label Copy Draft it renders). Independent spot-check pass (12 sampled records) found zero issues. Coverage is now 50/52 artifacts with `fields`, with the 2 non-coverage cases being deliberate, documented exemptions rather than gaps.
- **Referential integrity + browser-verified:** re-ran the validation script (zero dangling ids/refs across all core files); confirmed in-browser that the modal no longer renders an Example section and that a previously bare record (Receiving Log) now shows a populated, plausible Fields table ending cleanly at Related, with zero console errors.

**Item 10 completed — Implementation Detail card + modal upgrade, including full content backfill — done, browser-verified.** Deliberately done before item 9 (the large data audit/expansion phase): richer schema and a real destination for deeper content needed to exist before expanding the dataset further, and the pass also sharpened the content bar (`content-standards.md` §3a) that item 9 will need to follow. **Note:** the `example` field described below was removed in the correction entry above — this entry is kept as-written for the historical record of what originally shipped.
- **Schema:** `artifacts`, `materials`, and `systems` all gained four new optional fields per §3a: `description` (fuller prose beyond the existing one-liner), `example` (concrete illustrative content — since removed, see above), `fields` (array of `{name, description}`, only for records with real internal structure), and `relatedIds` (cross-references into artifacts/materials/systems/regulations). `content-standards.md` §3a documents the voice/length/sourcing bar for each; section 5's referential-integrity rule was extended to cover `relatedIds` the same as any other `*Ids` field.
- **UI:** new `app/js/render-detail-modal.js` — a reusable modal distinct from the existing source modal, showing kicker/name/description/fields-table/example/related-chips, with the same overlay-click/Escape/focus-on-open affordances as the rest of the site. `render-step.js`'s `listCard` now renders artifacts/materials/systems as clickable cards (keyboard-accessible, `role="button"`, `tabIndex`) when a record has any detail content; clicking opens the modal, and related chips re-open the modal on the related record without a page navigation. Regulations/agencies/roles cards were deliberately left as plain rows (not in scope, per the original item 10 plan).
- **Content backfill — full pass, all 72 records, not a subset:** ran as a 7-batch parallel research workflow (5 artifact batches + materials + systems), each batch agent reading `content-standards.md` and doing real web lookups for any new factual claim, same rigor bar as item 9a. Verified via an independent spot-check pass (parallel adversarial-style review agents) after merging into the JSON files. Real error caught and fixed: **FSIS Label Submission**'s citation for the 2023 generic-label-approval expansion rule was transcribed as "88 FR 2793" — corrected to the actual "88 FR 2798" after a reviewer agent re-checked it against the Federal Register. One transcription bug in the raw research output (a materials-collection id typo, `620aaecb-0b8f-4188-...` instead of the real `620aaecb-0b8f-4336-...` for Temperature Record) was caught by the referential-integrity script and fixed before commit.
- **Coverage confirmed programmatically:** 52/52 artifacts, 9/9 materials, 11/11 systems now have a `description`; zero dangling `relatedIds`/`sourceIds` across all four core files (extended validation script, same shape as prior passes — throwaway/scratchpad, rerun-worthy if the data changes again). No new `sources.json` entries were needed — all new claims were groundable against the 115 sources already added during item 9a.
- **Browser-verified:** clicking a card opens the modal with correct content at every tested depth, including the maximum-depth case (CCP Definition, 3 levels deep: primary flow → grouping flow → CCP monitoring's own nested flow) rendering a populated fields table, example, and related chips with zero console errors; a related chip correctly re-opens the modal on a different record (including one, Material / Product Specification, that has no detail fields of its own — confirmed it still opens cleanly with just name/description, since the modal doesn't require every field to be present); Escape and overlay-click both close the modal correctly.
...
9. **Large data audit & expansion phase — in progress.** Once existing content is verified/sourced (item 9a) and the detail-field shape and bar are established (item 10), this becomes the next major body of work. **First sub-pass done:** a full correctness/completeness audit across every collection (see section 10's newest entry) — 15 real findings fixed (2 new agencies, 2 new regulations, a stale vendor rebrand, 3 systems' empty vendor lists populated, all 4 threads' `stepIds` reconciled to bidirectional consistency, 2 genuine artifact-coverage gaps filled), zero referential-integrity errors remain. **Still open:** the actual expansion — more artifacts, materials, systems, regulations, and possibly more nested-flow depth — with new content sourced as it's written per `content-standards.md`. Expect this to surface real answers to the still-open gating question referenced in the historical section 3 discussion.
10. ~~Implementation Detail card + modal upgrade~~ — **done, browser-verified.** Deliberately sequenced *before* item 9 (reordering the original plan) since it sets the stage for item 9's expansion and sharpens the content bar first. See section 10's newest entry for the full account — schema extended on all three collections (`description`/`example`/`fields`/`relatedIds`), new `render-detail-modal.js` + clickable-card UI shipped, and a full content backfill written for all 52 artifacts, 9 materials, and 11 systems (100% coverage, not a representative subset). `content-standards.md` §3a now documents the bar for these fields.
11. ~~Vue.js migration~~ — **done, browser-verified.** Run out of plan order at the user's explicit request (ahead of item 9, not after it as originally sequenced) — confirmed via an explicit order-check prompt before starting. The vanilla ES-modules + hand-rolled hash router + manual re-render approach (phase 6) was ported to Vue 3 (Composition API, `<script setup>`), Vite, and Vue Router (hash mode), following the concise plan as drafted: data layer ported first as a `useData` composable (unchanged fetch/cache/path-id-resolution behavior), then one view at a time (flow view, step detail, nested flow, detail modal, source modal, search, sources page — the shared `listCard` helper split into `ListCard`/`ItemCard` components, which Vue naturally wants over a shared JS function), then routing last (hash-mode Vue Router with equivalent routes, preserving existing bookmarked-URL shapes), then CSS carried over as one global stylesheet (already theme-aware and not framework-specific, so left un-componentized per the plan's own guidance). Verified in an actual browser (Playwright): primary flow → nested flow at multiple depths → step detail at max depth (CCP monitoring, 3 levels) → detail modal open/close → search → sources page with live filtering — zero console errors. `npm run build` produces a clean static bundle (137KB JS / 11KB CSS gzip).
    - **Repo layout changed as a follow-up, same session:** the Vue app was initially scaffolded alongside the old `app/` (per the plan's own step 1) as `app-vue/`, verified independently, then — since it was a full replacement, not a second app — merged up into the repo root (`src/`, `public/`, `vite.config.js`, `package.json` at top level) and the old vanilla `app/` tree removed entirely (it was already gone from disk by the time of the merge; not deleted as part of this migration). Root `package.json`, `README.md`, and `.gitignore` were merged (project name/description/license preserved, scripts switched to Vite's `dev`/`build`/`preview`, ignore rules combined). This satisfies step 6 of the concise plan (delete the old vanilla files only once the Vue equivalent is verified) by way of the old files no longer existing to delete.
    - Not done as part of this pass: TypeScript adoption, Pinia (a small `useModals` composable covers the one case of cross-component modal-open state — two flags, not real shared app state, so Pinia wasn't reached for), and deploying the built output anywhere (item 7, still open).
