...
> upgrade) is now also done**, completed ahead of item 9 (the large data
> audit/expansion phase) by deliberate choice: item 10 sets the stage for
> item 9 (richer schema, a real place to put deeper content) and sharpens
> what "good" artifact/material/system content looks like before expanding
> the dataset further. Item 9 itself remains queued, not started. The
...
> [`content-standards.md`](content-standards.md) (item 8a, now also covering
> section 3a's detail-field rules from item 10) — read that before any new
> content work (item 9, or further backfilling).
...
9. **Large data audit & expansion phase — new, not yet started, comes after item 10.** Once existing content is verified/sourced (item 9a) and the detail-field shape and bar are established (item 10), this becomes the next major body of work: expand the data beyond what exists today — more artifacts, materials, systems, regulations, and possibly more nested-flow depth — with new content sourced as it's written per `content-standards.md`, using the same `description`/`example`/`fields`/`relatedIds` shape item 10 just established. Expect this to surface real answers to the still-open gating question referenced in the historical section 3 discussion. Not scoped further yet; treat as the next major phase now that item 10 is done.
